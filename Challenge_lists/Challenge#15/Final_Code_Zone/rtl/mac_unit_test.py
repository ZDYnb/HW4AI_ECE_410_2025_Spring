#!/usr/bin/env python3
"""
Cocotb test for MAC unit
Test S5.10 fixed-point multiply-accumulate functionality
"""
...
async def test_mac_debug_step_by_step(dut):
    ...
    # Step 2: Set inputs but not enable
    ...
    result = s5_10_to_float(dut.result.value.integer)
    dut._log.info(f"Set inputs but enable=0: result={result} (should be 0)")
    
    # Step 3: Correct timing - set on falling edge, read on falling edge
    dut._log.info("Step 3: First calculation 2.0 * 3.0 (correct timing)")
    
    await FallingEdge(dut.clk)  # Falling edge: set inputs
    dut.a.value = float_to_s5_10(2.0)  # 2.0
    dut.b.value = float_to_s5_10(3.0)  # 3.0
    dut.clear.value = 1
    dut.enable.value = 1
    
    # Now combinational logic starts calculation...
    await RisingEdge(dut.clk)   # Rising edge: sequential logic updates register
    await FallingEdge(dut.clk)  # Falling edge: read stable result
    
    result = s5_10_to_float(dut.result.value.integer)
    dut._log.info(f"First calculation result: {result} (expected 6.0)")
    
    # Step 4: Accumulate test - set and read on falling edge
    ...
    await RisingEdge(dut.clk)   # Rising edge: update register
    await FallingEdge(dut.clk)  # Falling edge: read result
    
    result = s5_10_to_float(dut.result.value.integer)
    dut._log.info(f"Result after accumulation: {result} (expected 6+1=7)")
    
    # Step 5: Negative number test - set and read on falling edge
    ...
    # Print input hex values for debugging
...
async def test_mac_accumulate(dut):
    ...
    await RisingEdge(dut.clk)
    dut.clear.value = 1
    await RisingEdge(dut.clk)  # Falling edge: read result
    dut.clear.value = 0
...
async def test_mac_enable_clear(dut):
    ...
    # Test that no accumulation when enable=0
    dut.clear.value = 1
    await FallingEdge(dut.clk)  # Falling edge: read result
    dut.clear.value = 0
...
async def test_mac_overflow(dut):
    ...
    # Test large values (close to S5.10 boundary)
    dut.enable.value = 1
