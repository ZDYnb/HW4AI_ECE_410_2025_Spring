...
async def test_mac_debug_step_by_step(dut):
    ...
    # 步骤3: 正确的时序 - 下降沿设置，下降沿读取
    dut._log.info("步骤3: 第一次计算 2.0 * 3.0 (正确时序)")
    
    await FallingEdge(dut.clk)  # 下降沿：设置输入
    dut.a.value = float_to_s5_10(2.0)  # 2.0
    dut.b.value = float_to_s5_10(3.0)  # 3.0
    dut.clear.value = 1
    dut.enable.value = 1
    ...
    # 现在组合逻辑开始计算...
    await RisingEdge(dut.clk)   # 上升沿：时序逻辑更新寄存器
    await FallingEdge(dut.clk)  # 下降沿：读取稳定结果
    
    result = s5_10_to_float(dut.result.value.integer)
    dut._log.info(f"第一次计算结果: {result} (期望6.0)")
    ...
    await RisingEdge(dut.clk)   # 上升沿：更新寄存器
    await FallingEdge(dut.clk)  # 下降沿：读取结果
...
async def test_mac_accumulate(dut):
    ...
    await RisingEdge(dut.clk)
    dut.clear.value = 1
    await RisingEdge(dut.clk)  # 下降沿：读取结果
    dut.clear.value = 0
